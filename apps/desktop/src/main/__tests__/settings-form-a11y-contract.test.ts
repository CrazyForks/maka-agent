...
describe('Settings form accessibility labels', () => {
  it('keeps Settings secondary surfaces close to QoderWork card geometry', async () => {
    ...
    assert.match(catalogTabsButton, /border-radius:\s*8px;/, 'Settings model category tabs should use QoderWork rounded-lg geometry');
    assert.match(providerCatalogCard, /border-radius:\s*8px;/, 'Settings provider catalog cards should use QoderWork rounded-lg geometry');
    ...
    assert.match(providerCatalogBadge, /border-radius:\s*4px;/, 'Provider catalog badges should use compact squared QoderWork-style corners, not pills');
    assert.match(providerUnavailableBadge, /border-radius:\s*4px;/, 'Provider unavailable badges should use compact squared QoderWork-style corners, not pills');
    assert.match(providerExperimentalBadge, /border-radius:\s*4px;/, 'Provider experimental badges should use compact squared QoderWork-style corners, not pills');
